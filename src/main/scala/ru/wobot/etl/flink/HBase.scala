...
object HBase extends App {
  ...
  val toAdd = env.createInput(new DocumentInputFormat(HBaseConstants.T_PROFILE_TO_ADD))
  val saved = env.createInput(new DocumentInputFormat(HBaseConstants.T_PROFILE))
...
//  admin.truncateTable(HBaseConstants.Tables.PROFILE_TO_ADD, true)
  //admin.enableTable(HBaseConstants.Tables.PROFILE_TO_ADD)
  ...
  class DocumentInputFormat(val tableName: String) extends TableInputFormat[Doc] {
